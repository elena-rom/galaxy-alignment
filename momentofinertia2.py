...
def toXY(name, coords, bcgcoords): 
    ...
    for i in range(len(coords)):
        ...
        #x = math.cos(math.radians(ranew))*math.cos(math.radians(decnew))
        #y = math.sin(math.radians(ranew))*math.cos(math.radians(decnew))
...
# def findcom(name, bcgcoords):
#     for i in bcgcoords:
#         if name == i[0]:
#             coords = [[i[1], i[2]]]
#             xy = toXY(coords)
#             return xy[0]  ### multiplies the RA by cos(dec)
#             #return [i[1], i[2]] ### if not accounting for that
#     return None
